      ...
      .then((resposta) => {
        ...
        if (
          resposta.data.perfil?.permissoesPerfis.filter((modulo: any) => {
            if (modulo.nome === "Atributos Imóveis") {
              return modulo;
            }
          }).length === 0
        ) {
    ...
    profile?.perfil?.permissoesPerfis.filter((modulo) => {
      if (modulo.nome === "Atributos Imóveis") {
        const valor = modulo.permissoes.filter((permissao) => {
          if (
            permissao.nome === "Atualizar Atributo" ||
            permissao.nome === "Deletar Atributo"
          ) {
            return true;
          }
        });
    ...
    profile?.role === "DONO"
      ...
      : profile &&
        ...
        profile?.perfil?.permissoesPerfis.filter((modulo) => {
          if (modulo.nome === "Atributos Imóveis") {
            const valor = modulo.permissoes.filter((permissao) => {
              if (permissao.nome === "Atualizar Atributo") {
                return true;
              }
            });
            if (valor.length > 0) {
      ...
      : profile &&
        ...
        profile?.perfil?.permissoesPerfis.filter((modulo) => {
          if (modulo.nome === "Atributos Imóveis") {
            const valor = modulo.permissoes.filter((permissao) => {
              if (permissao.nome === "Deletar Atributo") {
                return true;
              }
            });
            if (valor.length > 0) {
    ...
    <Layout>
      {openExcluir === true && (
        ...
        >
          ...
          >
            <p>
              Tem certeza que quer apagar o atributo:{" "}
      ...
      {openCreateAtributo === true && (
        ...
        >
          ...
          >
            <Form.Item
              name="atributo"
              rules={[{ required: true, message: "Por favor insira o nome do atributo" }]}
              label={<p className="font-bold text-md">Nome do Atributo</p>}
            >
              <Input />
            </Form.Item>
            <div className="flex justify-center gap-4 pb-4 mt-8">
      ...
      <div className="mx-6 pt-6 pb-2 border-gray-300 border-b-2 flex flex-col sm:flex-row justify-between ">
        <div className="flex items-center gap-2 py-2 justify-center">
          ...
          <FaLongArrowAltRight />
          <p className="sm:text-2xl   font-bold">Atributo Imóveis</p>
        </div>
        <div className="flex justify-center items-center bg-white px-2 border-2 rounded-lg focus-within:border-azul-evler">
          <input
            ...
            placeholder="Busque por atributos de imóveis"
          />
          <IoMdSearch size={20} />
        </div>
        <div>
          ...
          profile?.perfil?.permissoesPerfis.filter((modulo) => {
            if (modulo.nome === "Atributos Imóveis") {
              const valor = modulo.permissoes.filter((permissao) => {
                if (permissao.nome === "Cadastrar Atributo") {
                  return true;
                }
              });
              if (valor.length > 0) {
      ...
      <div className="px-6 mt-4 space-y-6 h-[calc(100vh-170px)] overflow-y-auto custom-scrollbar">
        {filteredDataWithoutEmptyConfigurations.length > 0 ? (
          ...
          <p className="text-xl font-bold">Nenhum atribulo localizado</p>
        )}
      </div>
    </Layout>
  );
