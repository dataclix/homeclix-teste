...
import { atualizarImovelAtom, openCreateImovelAtom } from "@/pages/painel-administrativo";
import { api } from "@/services/apiClient";
import { Checkbox, Form, Input, Image, InputNumber, Modal, Popconfirm, Select, SelectProps, Switch } from "antd";
...
export default function ModalCreateImovel() {
    const [open, setOpen] = useAtom(openCreateImovelAtom)
    const [form] = Form.useForm();
    const [loadingImagens, setLoadingImagens] = useState<boolean>(false)
    ...
    const onFinish = (values: any) => {
        ...
        api.post('imoveis', saida).then((resposta) => {
            ...
            api.post('imoveis/imagens/upload/' + resposta.data.id, data).then((resposta) => {
            ...
            console.log(err)
            toast.error('ID do Imóvel ja existe!')
        })

    }
    return (
        <Modal centered width={1000} open={open} onCancel={() => setOpen(false)} footer={false} title={<p className="text-2xl font-libre  py-3 px-6 bg-verde text-white rounded-t-lg">Criar Imóvel</p>} closeIcon={<IoMdClose size={24} color="white" className="" />}>
            {loadingImagens === true ? <LoadingScreen heigth={"600px"} message="Subindo as imagens" /> : (<Form form={form} layout="vertical" onFinish={onFinish} onFinishFailed={onFinishFailed}>
                <div className="h-[530px] overflow-y-auto custom-scrollbar">
                    <div className="px-2 ">
                        ...
                        <div className={`w-full border-[1px]  border-verde rounded-b-md transition-all duration-1000 px-6  ${openInformacaoBasica === true ? 'h-full sm:h-[270px] opacity-100' : 'opacity-0 hidden h-0'}`}>
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 pt-4">
                                ...
                                <Form.Item className="" rules={[{ required: true, message: 'Por favor insira a modalidade' }]} name='modalidade' label={<p className="font-bold text-base">Modalidade</p>}>
                                    <Select options={[{ label: 'Venda', value: 'VENDA' }, { label: 'Aluguel', value: 'ALUGUEL' }]} />
                                </Form.Item>
                                <Form.Item className="" rules={[{ required: true, message: 'Por favor insira o tipo' }]} name='tipo' label={<p className="font-bold text-base">Tipo</p>}>
                                    <Select
                                        options={optionsTipos}
                                        onChange={(e, option) => onChangeTipo(e, option)}
                                    />
                                </Form.Item>
                                <Form.Item className="" rules={[{ required: true, message: 'Por favor insira o subTipo' }]} name='subTipo' label={<p className="font-bold text-base">SubTipo</p>}>
                                    <Select disabled={(tipoNull === true) ? true : false} onChange={(e, option) => onChangeSubTipo(e, option)} options={optionsSubTipos} />
                                </Form.Item >

                            </div>
                            <div className="grid grid-cols-5 gap-x-4">
                                <Form.Item className="col-span-2" rules={[{ required: true, message: 'Por favor insira o proprietário' }]} name='cliente' label={<p className="font-bold text-base">Proprietário</p>}>
                                    <Select
                            ...
                            <div className="grid grid-cols-4 gap-x-4">
                                <Form.Item name='olx' initialValue={false} valuePropName="checked" label={<p className="font-bold text-base">OLX</p>}>
                                    <Checkbox>Sim</Checkbox>
                                </Form.Item>
                                <Form.Item name='terra' initialValue={true} valuePropName="checked" label={<p className="font-bold text-base">Terra dos Imóveis</p>}>
                                    <Checkbox >Sim</Checkbox>
                                </Form.Item>
                                <Form.Item name='permuta' initialValue={false} valuePropName="checked" label={<p className="font-bold text-base">Aceita Permuta?</p>}>
                                    <Checkbox >Sim</Checkbox>
                                </Form.Item>
                            </div>
                        </div>
                    ...
                    <div className="px-2 mt-4">
                        ...
                        <div className={`w-full   border-[1px]  border-verde rounded-b-md transition-all duration-1000 px-6 ${openLocalizacao === true ? 'opacity-100 h-full sm:h-[370px]' : ' opacity-0 hidden h-0'} `}>
                            <div className="grid sm:grid-cols-4 grid-cols-2 gap-x-4 pt-4">
                                <div>
                                    ...
                                    <input placeholder="00000-000" required value={cep ? cep : ''} onChange={handleCepChange} className="border-[1px] border-gray-300 rounded-md w-full h-8 px-2 mt-2" />
                                </div>
                    ...
                    <div className="px-2 mt-4">
                        ...
                        <div className={`w-full  border-[1px]  border-verde rounded-b-md transition-all duration-1000 px-6  ${openCaracteristicaImovel === true ? 'opacity-100 h-full sm:h-[400px] ' : 'opacity-0 hidden h-0'}`}>
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-4 pt-4">
                                ...
                                <Form.Item
                                    name="quartos"
                                    rules={[{ required: true, message: "Por favor insira a quantidade de quartos" }]}
                                    className="w-full"
                                ...
                                <Form.Item
                                    name="banheiros"
                                    rules={[{ required: true, message: "Por favor insira a quantidade de banheiros" }]}
                                    className="w-full"
                                ...
                                <Form.Item
                                    name="garagens"
                                    rules={[{ required: true, message: "Por favor insira a quantidade de garagens" }]}
                                    className="w-full"
                    ...
                    <div className="px-2 mt-4 ">
                        ...
                        <div className={`w-full   border-[1px]  border-verde rounded-b-md transition-all duration-1000 px-6 ${openInformacaoVenda === true ? 'h-full sm:h-[270px] opacity-100' : ' hidden h-0 opacity-0'}`}>
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 pt-4">
                                ...
                                <Form.Item label={<p className="font-bold text-base">2º Captador</p>}>
                                    <Select
                                        options={optionsCaptador}
                                        showSearch
                                ...
                                <Form.Item label={<p className="font-bold text-base">Filial</p>}>
                                    <Select
                    ...
                    <div className="px-2 mt-4 mb-6">
                        ...
                        <div className={`w-full   border-[1px] border-verde rounded-b-md transition-all duration-1000 px-6 ${openInformacaoAdicional === true ? 'h-[1280px] sm:h-[1050px] opacity-100 block ' : 'h-0 hidden opacity-0'}`}>
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-4 pt-4">
                            ...
                            <div>
                                ...
                                <div className=" rounded-lg  py-4  h-[310px] ">
                                    ...
                                    <ReactQuill onChange={onChangeDescricao} theme="snow" className="mt-4   bg-white   " style={{ height: '200px' }} />
                                </div>
                                <div className=" rounded-lg  py-4  h-[310px] ">
                                    <p className="text-xl font-bold font-andika text-start mt-2">Restrições do imóvel</p>
                                    <ReactQuill onChange={onChangeRestricao} theme="snow" className="mt-4   bg-white   " style={{ height: '200px' }} />
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="px-2 mt-4 mb-6">
                        ...
                        <div className={`  border-[1px] border-verde rounded-b-md transition-all duration-1000 px-2 ${openImagem === true ? ' opacity-100 block  ' : 'h-0 hidden opacity-0'}`}>
                            {converterImagem === true ? <LoadingScreen heigth="200px" message={'convertendo imagens ' + quantidade + ' de ' + total} /> :
                                <>
                                    <div className="w-[62vw] overflow-x-hidden whitespace-nowrap mt-4">
                                                                ...
                                                                <Draggable key={item.caminho} draggableId={item.caminho} index={index}>
                                                                    {(provided) => (
                                                                        <div
                                                                            ...
                                                                            className="w-[14.5vw] h-[10vw] overflow-y-hidden"
                                                                        >
                                                                            <div className="relative h-0 w-0 z-30 left-2 bottom-[45%] ">
                                                                                <Popconfirm
                                                                                    ...
                                                                                    cancelText="Não"
                                                                                >
                                                                                    <button type="button" className="bg-white w-6 h-6 flex justify-center items-center rounded-full">
                                    ...
                                    <div className="flex justify-end py-2 pr-4 w-full">
